...
def main():
    run=N/'run_v1'; out=N/'cpu_recovery_v4'; report=N/'report_cpu_v4'
    ...
    state={'pid':os.getpid(),'status':'wait_cpu_smoke','training_steps':0,'gpu_hours':0,'processes':[],
           'scripts_sha256':{str(p):sha256(p) for p in (Path(__file__),N/'scripts/region_cpu.py',N/'scripts/region_diagnostics.py',N/'scripts/summarize_cpu.py')}}
    ...
    try:
        ...
        for field,prior in (('region_dino','garment_dino'),('region_hf_lpips','garment_hf_lpips')):
            ...
            if diff > (1e-5 if field=='region_dino' else 1e-3):raise ValueError('CPU/GPU discrepancy exceeds disclosed numeric bound')
        ...
        state['processes'].append(execute('report',[PY,'-u',N/'scripts/summarize_cpu.py',
        ...
        with (report/'REPORT.md').open('a') as f:
            ...
            f.write('CPU小样本与全量重算差≤1e-6；完整衣服DINO与原GPU差≤1e-5、HF-LPIPS差≤1e-3；两种设备不宣称逐值相等。\n')

...
def estimator_sensitivity(rows):
    result={}
    for branch in ('matched','mismatched','original'):
        for tau in (.1,.4,.8):
            a=[r for r in rows if (r['branch'],r['tau'],r['k'])==(branch,tau,4)]
            keys={pairkey(r) for r in a}
            b=[r for r in rows if (r['branch'],r['tau'],r['k'])==(branch,tau,1) and pairkey(r) in keys]
            result[f'{branch}__t{tau}__k4-minus-k1_same_subset']={f:paired_effect(a,b,f) for f in FIELDS}
    return result
...
def main():
    ...
    result={'formal_result':False,'training_steps':0,'A_predictions':len(rows),'B_predictions':len(br),
        ...
        'A_estimator_sensitivity':estimator_sensitivity(rows),

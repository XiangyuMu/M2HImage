import unittest
from summarize_a_b import paired_effect, weights_for, estimator_sensitivity
...
class PairedTests(unittest.TestCase):
    ...
    def test_estimator_uses_identical_subset(self):
        rows=[{**r,'branch':'matched','tau':.8,'k':1,'id_penalized':0.} for r in self.rows()]
        rows += [{**r,'k':4,'id_penalized':1.} for r in rows[:2]]
        result=estimator_sensitivity(rows)['matched__t0.8__k4-minus-k1_same_subset']['id_penalized']
        self.assertEqual(result['expected'],2)
        self.assertEqual(result['delta_mean'],1.)

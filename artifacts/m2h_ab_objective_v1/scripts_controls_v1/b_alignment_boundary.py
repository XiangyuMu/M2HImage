...
def measure(source, base, output, mask, config=Config(), context=None):
    ...
    for name in ("garment", "interior", "boundary", "strong_texture"):
        roi = regions[name]
...
def summarize_calibration(rows):
    ...
    for kind in ("splice_offset", "misalignment"):
                ...
                try:
                    ...
                    for kind, severity in calibration_cases():
                        ...
                        cr = {"mid": mid, "kind": kind, "severity": severity,
                              "severity_unit": "u8_RGB_offset" if kind == "splice_offset" else "native_pixels" if kind == "misalignment" else "none",

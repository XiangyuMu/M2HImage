...
def select_dev_pairs(rows, mids, embedding_ids, vectors, image_audit,
                     expected_m=64, max_refs=128, min_refs=2):
    ...
    if not 2 <= min_refs <= max_refs <= 128:
    ...
    for i, mid in enumerate(mids):
        ...
        ordered = sorted(eligible, key=lambda j: ((j in selected) if len(selected)<min_refs else (j not in selected), uses[j], stable_order(mid+':'+j)))
        chosen = None
        for a in ordered:
    ...
    if len(selected) < min_refs:
        raise ValueError('Reference coverage below preregistered minimum')
    payload = {'schema_version': PROTOCOL, 'pairs': pairs,
               'minimum_reference_files': min_refs,
...
def main():
    ...
    payload = select_dev_pairs(rows, mids, ids, vectors, images, min_refs=32)

...
class SelectionTests(unittest.TestCase):
    ...
    def test_minimum_reference_coverage(self):
        result = select_dev_pairs(self.rows, self.ids[:2], self.ids, self.vectors,
                                  self.audit, expected_m=2, min_refs=4)
        self.assertEqual(len(result['reference_ids']), 4)
        with self.assertRaisesRegex(ValueError, 'coverage below'):
            select_dev_pairs(self.rows, self.ids[:2], self.ids, self.vectors,
                             self.audit, expected_m=2, min_refs=5)

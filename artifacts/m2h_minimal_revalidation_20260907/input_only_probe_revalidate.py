...
def check_dataset_read(path, root, allowed):
    resolved = Path(path).resolve()
    if resolved.is_relative_to(Path(root).resolve()) and resolved not in allowed:
        raise PermissionError('input-only dataset read denied: ' + str(resolved))


def install_read_guard(root, allowed):
    ...
    def hook(event, args):
        if event in ('subprocess.Popen', 'os.system', 'os.posix_spawn', 'os.exec'):
            ...
            raise PermissionError('Subprocesses disabled in input-only probe')
        if event == 'open' and isinstance(args[0], (str, bytes, os.PathLike)):
            path = Path(os.fsdecode(args[0])).resolve()
            if path.is_relative_to(root):
                try:
                    check_dataset_read(path, root, allowed)
                except PermissionError:
                    events['denied_reads'].append(str(path))
                    raise
                events['dataset_reads'].append(str(path))
...
def source_path(root, role, sid):
    ...
    for suffix in ('.png', '.jpg', '.jpeg'):
        ...
        if p.is_file():
            if not p.resolve().is_relative_to(base / role):
...
def main():
    ...
    p.add_argument('--config', type=Path,
                   default=Path('configs/spatial_hair_ab_space.yaml'),
                   help='model config used to load the checkpoint')
    p.add_argument('--checkpoint', type=Path, default=None,
                   help='checkpoint directory; defaults to the historical spatial checkpoint')
    args = p.parse_args()
    if args.limit < 1:
    ...
    cfg_path = args.config if args.config.is_absolute() else args.repo / args.config
    ...
    ckpt = (args.checkpoint if args.checkpoint is not None else
            args.root / 'phase1/phase1_spatial_hair_ab_space_r16_8400_768x1024/checkpoints/final')
    allowed = [textpath] + [old / 'samples' / (j + '.npz') for j in jids]
    allowed += [source_path(args.root, role, mid) for mid in mids
                for role in ('images/mannequin', 'dwpose/with_head/mannequin')]
    allowed += [f for f in ckpt.rglob('*') if f.is_file()]
    read_events = install_read_guard(args.root, allowed)
    denied_bait = args.root / 'images/human' / (mids[0] + '.jpg')
    try:
        with denied_bait.open('rb'):
            pass
    except PermissionError:
        bait_passed = True
    else:
        raise RuntimeError('Target-file denial test failed')
    audit = {'formal_result': False, 'status': 'building', 'roles': [],
             ...
             'limitations': ['Old I-only feature lineage is code-traced, not rebuilt here.',
                            'Python open/subprocess audit does not intercept arbitrary native library syscalls.',
                            'Historical initialization is not clean final training.'],
             'read_guard': read_events, 'target_denial_test_passed': bait_passed,
             'allowed_dataset_paths': [str(Path(f).resolve()) for f in allowed],
